...
public class CaloriesGraph extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toLoseOneLb = dailyCaloricIntake - 1000;
        toGainOneLb = dailyCaloricIntake + 1000;

        caloriesText = (TextView) findViewById(R.id.CaloriesTextView);

        if (calories != null) {
            caloriesText.setText("Daily Caloric intake to maintain weight is " + dailyCaloricIntake + ". In order to lose one Kg per week, the Daily Caloric Intake needs to be adjusted to " +
                    toLoseOneLb + ". In order to gain one Kg per week, the Daily Caloric Intake must be adjusted to " + toGainOneLb + ".");
        }
